...
recipe.ingredients.forEach(element => {
  let ingredientToSlice = element ;
  let indexToFirstSlice = ingredientToSlice.indexOf(' ');
  // console.log("indexToFirstSlice", indexToFirstSlice)
  let firstSlice = ingredientToSlice.slice(indexToFirstSlice +1);
  let indexToSecondSlice = firstSlice.indexOf(' ');
  // console.log("indexToSecondSlice", indexToSecondSlice)
  let secondSlice = firstSlice.slice(indexToSecondSlice+1);
...
const splitFoods = (recipe) => {
  ...
  // Solution code here...
  recipe.ingredients.forEach(element=>{
    let toRemoveUndesire = element.split(' ') ;
    // console.log('wish it array', toRemoveUndesire)
    toRemoveUndesire.shift(); // delete number 
    toRemoveUndesire.shift(); // delete the unit 
    let returnToString = toRemoveUndesire.join(' ');
    result.push(returnToString);
...
for( var i = 0 ; i < recipe.steps.length ; i++){
     let theDesireItem = recipe.steps[i] ;
    //  console.log('theDesireItem', theDesireItem)
     let mai = theDesireItem.split(' ')
     result.push(mai[0])
...
const removeVowels = (str) => {
  // Solution code here...
   ...
   str = str.map(character => {
    if(/[aeiouAEIOU]/.test(character)){ /// test from regular expresiion 
       character = ""
...
const extractVowels = (str) => {
  ...
  str = str.map(character => {
   if(/[aeiouAEIOU]/.test(character)){ /// test from regular expresiion 
    vowelStr = vowelStr +character ;
    vowelStr.split().sort().join();
    console.log('vowelstring', vowelStr)
      character = ""
      // return vowelStr 
   } else {return character}
}).join("") ; 
